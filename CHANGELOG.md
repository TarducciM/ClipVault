...
- **Riconoscimento URL/email/colore**: una voce di testo che è per intero un URL http/https o un indirizzo email ora offre "Apri nel browser"/"Scrivi email" nel menu contestuale (tasto destro), invece delle sole azioni generiche. Un testo che è un colore esadecimale (`#rrggbb` o `#rgb`) mostra un riquadro colorato al posto della solita icona, sia nella lista sia nell'anteprima grande. Verificato dal vivo: colore mostrato correttamente in lista con la tinta esatta copiata; "Apri nel browser" ha aperto realmente la pagina nel browser di sistema (titolo della finestra Chrome cambiato di conseguenza); "Scrivi email" compare correttamente nel menu ma l'apertura del client di posta non è stata confermata visivamente — sulla macchina di sviluppo non risulta un gestore `mailto:` predefinito, quindi Windows non apre nulla di osservabile; il comando Rust (`open_url`) è lo stesso identico usato per il caso http già confermato funzionante, quindi non è un dubbio sul codice.
- **Snippet fissi**: nuovo pulsante 🗒 accanto alla ricerca per passare a una lista separata di testi salvati a mano (non provenienti dagli appunti), utile per firme email, comandi ricorrenti, ecc. Si aggiungono scrivendo nella barra di ricerca (che in questa vista diventa un campo "nuovo snippet") e premendo Invio; click per ricopiarli negli appunti, × per eliminarli. Tabella dedicata nel database, esplicitamente esclusa dalla pruning automatica della cronologia (verificato con un test unitario dedicato). Verificato dal vivo: creato uno snippet, ricopiato con un click (contenuto confermato leggendo gli appunti di sistema), eliminato con la ×.
- **Esporta/importa cronologia**: nuova sezione "Backup" nelle Impostazioni con due pulsanti che aprono le finestre di sistema per salvare/scegliere un file `.json`. L'export include cronologia e snippet, con le immagini incorporate come base64 (file autonomo, portabile su un'altra macchina senza dover copiare anche la cartella immagini). L'import ripristina le voci mantenendo data di creazione e stato preferito originali (una funzione dedicata nel database salta di proposito la pruning automatica usata per le catture live, per non perdere pezzi di un backup appena ripristinato). Verificato dal vivo end-to-end: esportata la cronologia reale (85 voci, JSON valido, un'immagine campione decodificata dal base64 e confermata PNG valido), poi importato un file JSON di prova con 2 voci (una preferita, una no) e 1 snippet — tutti e tre ritrovati nel database con id nuovi ma data di creazione e stato preferito esattamente quelli del file, non quelli del momento dell'importazione.
- Nota sui limiti della verifica in questa sessione: interagire con le finestre di dialogo native "Salva con nome"/"Apri" via automazione si è rivelato particolarmente ostico — la finestra di questa stessa sessione di lavoro risultava spesso in cima allo z-order sopra ai dialoghi nativi (coprendo l'intero schermo), bloccando i click simulati. Risolto abbassando temporaneamente quella finestra in fondo allo z-order (`SetWindowPos`/`HWND_BOTTOM`, non distruttivo/reversibile) solo per la durata dell'interazione col dialogo. Non un problema del codice di ClipVault.
- Tutti i dati di test (voci ed eventuali snippet inseriti per la verifica) sono stati ripuliti dal database reale al termine, lasciando la cronologia esattamente come si trovava prima della sessione di verifica (a parte una voce copiata dall'utente stesso nel frattempo, lasciata intatta).
...
- Il codice di rendering dell'anteprima (testo, immagine, tabella file/zip) era duplicato nel popup — estratto in `src/preview-render.js`, condiviso tra il popup e la nuova finestra, così restano visivamente identici senza mantenere due copie.
- **Anteprima contenuto per file di testo e codice** (txt, md, json, c/cpp/h/hpp, cs, java, go, rs, py, html, css, js/ts, ecc — lista ampia dei formati più comuni) e **PDF** (testo estratto via `pdf-extract`, non l'impaginazione visiva): prima l'anteprima file mostrava solo nome/dimensione/data/hash, ora per questi tipi mostra anche il contenuto in un riquadro monospazio, troncato a 200 KB con indicazione "(troncato)" se il file è più lungo. Rispetta la stessa soglia "dimensione massima file" già usata per hash/immagini (sopra la soglia, niente anteprima contenuto). Verificato dal vivo con un file `.cpp` e un `.pdf` reali: contenuto mostrato correttamente sia nel popup che nella finestra grande.
- **Zoom sulle immagini**: rotellina del mouse per ingrandire/rimpicciolire (con overflow/scrollbar per spostarsi), doppio click per resettare. Funziona sia nell'anteprima del popup sia nella finestra grande, essendo condiviso lo stesso codice di rendering. Verificato dal vivo: zoom in progressivo confermato via screenshot, reset al doppio click confermato.
- Nota sui limiti della verifica in questa sessione: individuare le coordinate esatte del pulsante "⛶" via screenshot per l'anteprima di un'immagine si è rivelato particolarmente delicato — il box dell'anteprima è centrato verticalmente e la sua altezza (quindi la posizione dell'header) cambia parecchio in base al contenuto (un'immagine piccola produce un box molto più basso di una tabella file con anteprima codice). La funzionalità di apertura nella finestra grande è comunque verificata a fondo su testo/cpp/zip/pdf (stesso identico pulsante e codice, nessuna logica specifica per le immagini), quindi il gap è solo nella verifica visiva diretta per il caso immagine, non un dubbio sul funzionamento.
